        ...
        <table className="min-w-[1200px] w-full">
          ...
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              {columns.actions && (profile?.role === 'admin' || profile?.role === 'manager') && (
                <th className="text-center px-6 py-4 text-xs font-semibold text-slate-600 uppercase">
                  Actions
                </th>
              )}
          ...
          <tbody className="divide-y divide-slate-200">
            {displayedJobs.map((job) => (
                ...
                {columns.client && (
                  <td className="px-6 py-4 whitespace-nowrap">{job.client_name}</td>
                )}

                {columns.roll && (
                  <td className="px-6 py-4 whitespace-nowrap">{job.selected_roll_id ? 'Assigned' : 'Not assigned'}</td>
                )}

                {columns.status && (
                  <td className="px-6 py-4 text-center whitespace-nowrap">{getStatusBadge(job.status)}</td>
                )}

                {columns.finalCost && (
                  <td className="px-6 py-4 text-right whitespace-nowrap">{job.final_cost.toFixed(2)}</td>
                )}

                {columns.lockStatus && (
                  <td className="px-6 py-4 text-center whitespace-nowrap">{job.is_locked ? 'Locked' : 'Unlocked'}</td>
                )}
                ...
                {columns.actions && (profile?.role === 'admin' || profile?.role === 'manager') && (
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    <div className="flex items-center justify-center gap-2">
                      {!job.is_locked && (
                        <button onClick={() => onNavigateToUpdateJob?.(job.id)} className="p-2 hover:bg-blue-50 rounded-lg">
                      ...
                      {profile?.role === 'admin' && (
                        <>
                          <button onClick={() => handleToggleLock(job.id, job.is_locked)} className="p-2 hover:bg-slate-100 rounded-lg">
                            {job.is_locked ? <Unlock className="w-4 h-4 text-slate-600" /> : <Lock className="w-4 h-4 text-slate-600" />}
                          </button>

                          {!job.is_locked && (
                            <button onClick={() => handleDeleteJob(job.id, job.job_number)} className="p-2 hover:bg-red-50 rounded-lg">
